...
function App() {
    ...
    const [userProfile, setUserProfile] = useState({ name: 'Đang tải...', username: '', photoUrl: '' });
    ...
    const theme = { bg: '#0F0F0F', cardBg: '#1C1C1E', gold: '#F4D03F', textLight: '#FFFFFF', textDim: '#8E8E93', border: '#333333', green: '#34C759', red: '#FF3B30', blue: '#5E92F3', premium: '#E0B0FF' };
    ...
    // ... (Giữ nguyên useEffect và các hàm fetchUserData, handleCheckIn, v.v.) ...
    useEffect(() => {
        if (!unlockDateMs) return;
        const interval = setInterval(() => {
            const now = new Date().getTime();
            const distance = unlockDateMs - now;
            if (distance <= 0 || balance >= 1500) { setIsUnlocked(true); setTimeLeft({ days: 0, hours: 0, mins: 0 }); } 
            else { setIsUnlocked(false); setTimeLeft({ days: Math.floor(distance / (86400000)), hours: Math.floor((distance % (86400000)) / (3600000)), mins: Math.floor((distance % (3600000)) / (60000)) }); }
        }, 1000);
        return () => clearInterval(interval);
    }, [unlockDateMs, balance]);

    const fetchUserData = (uid: string) => { /* ... Giữ nguyên ... */ 
        fetch(`${BACKEND_URL}/api/user?id=${uid}`).then(res => res.json()).then(data => {
            setBalance(data.balance || 0);
            if (data.wallet) setWallet(data.wallet); if (data.gatecode) setGatecode(data.gatecode);
            if (data.fullName) setFullName(data.fullName); if (data.email) setEmail(data.email); if (data.phone) setPhone(data.phone);
            ...
            const daysLimit = data.isPremium ? 7 : 15; setLockDaysLimit(daysLimit);
            const joinMs = data.joinDate ? new Date(data.joinDate).getTime() : new Date("2026-02-22T00:00:00Z").getTime();
            setUnlockDateMs(joinMs + (daysLimit * 86400000));
            ...
            setTasks({ readTaskDone: lastDaily === todayStr, shareTaskDone: lastShare === todayStr, youtubeTaskDone: data.youtubeTaskDone || false, facebookTaskDone: data.facebookTaskDone || false });
        }).catch(err => console.error("Lỗi:", err));
    };

    useEffect(() => { /* ... Giữ nguyên ... */
        const tg = (window as any).Telegram?.WebApp;
        if (tg) { tg.ready(); tg.expand(); const user = tg.initDataUnsafe?.user; if (user) { const uid = user.id.toString(); setUserId(uid); setUserProfile({ name: `${user.first_name || ''} ${user.last_name || ''}`.trim(), username: user.username ? `@${user.username}` : '@nguoidung', photoUrl: user.photo_url || '' }); fetchUserData(uid); } }
    ...
    // ... (Hàm tính Rank & Logic cũ giữ nguyên, thêm helper lấy rank) ...
    ...
    const dummyUsers = [ { firstName: 'Vũ', lastName: 'Dũng', referralCount: 65 }, { firstName: 'Mai', lastName: 'Thiều Thị', referralCount: 60 }, { firstName: 'LINH', lastName: 'NGUYEN', referralCount: 47 }, { firstName: 'Minh', lastName: 'Ngọc Hoàng', referralCount: 33 }, { firstName: 'PHƯƠNG', lastName: 'ANH PHÙNG', referralCount: 27 }, { firstName: 'Nông', lastName: 'Mao', referralCount: 12 }, { firstName: 'Support', lastName: '', referralCount: 11 }, { firstName: 'OSAKA', lastName: 'CHAU HUYNH', referralCount: 10 }, { firstName: 'Trinh', lastName: 'Lê', referralCount: 9 }, { firstName: 'Lý', lastName: 'Hà', referralCount: 8 } ];
    if (displayBoard.length < 10) { const needed = 10 - displayBoard.length; displayBoard = [...displayBoard, ...dummyUsers.slice(0, needed)]; displayBoard.sort((a, b) => b.referralCount - a.referralCount); }
    let wealthBoard = displayBoard.slice(0, 10).map((user, index) => { let estimatedTotal = (user.referralCount * 25) + 300 + (10 - index) * 50; if (user.referralCount === referrals && user.firstName === userProfile.name.split(' ')[0]) { estimatedTotal = balance + (referrals * 25) + (checkInStreak * 5) + 50; } return { ...user, totalEarned: Math.round(estimatedTotal * 10) / 10 }; });
    wealthBoard.sort((a, b) => b.totalEarned - a.totalEarned);

    // ... (Giữ nguyên logic tính myRank, vipLevel, wreathColor, glow) ...
    let myRank = 0; if (referrals > 0) { const strictlyBetter = displayBoard.filter(u => u.referralCount > referrals).length; myRank = strictlyBetter + 1; }
    let vipLevel = "Tân Binh 🥉"; let wreathColor = "#8E8E93"; let glow = "none";
    ...
    // ... (Giữ nguyên các hàm handleCheckIn, handleClaimGiftCode, handleSaveWallet, handleWithdraw, handleCopyLink, handleClaimMilestone, startTask, claimTaskApp) ...
    const handleCheckIn = () => { /* ... Giữ nguyên ... */ fetch(`${BACKEND_URL}/api/checkin`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId }) }).then(res => res.json()).then(data => { if (data.success) { setBalance(data.balance); setLastCheckIn(data.lastCheckInDate); setCheckInStreak(data.streak); alert(`🔥 Điểm danh thành công!`); } else { alert(data.message); } }).catch(() => alert("Lỗi mạng!")); };
    const handleClaimGiftCode = () => { /* ... Giữ nguyên ... */ fetch(`${BACKEND_URL}/api/claim-giftcode`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, code: giftCodeInput }) }).then(res => res.json()).then(data => { if (data.success) { setBalance(data.balance); setGiftCodeInput(''); alert(`🎉 Chúc mừng! +${data.reward} SWGT`); } else { alert(data.message); } }).catch(() => alert("Lỗi!")); };
    const handleSaveWallet = () => { /* ... */ fetch(`${BACKEND_URL}/api/save-wallet`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, wallet, gatecode, fullName, email, phone }) }).then(() => alert('✅ Đã lưu!')); };
    const handleWithdraw = () => { /* ...Logic 1500 SWGT giữ nguyên... */ if (!isUnlocked && balance < 1500) return alert("Chưa đủ điều kiện!"); fetch(`${BACKEND_URL}/api/withdraw`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, amount: Number(withdrawAmount), withdrawMethod }) }).then(res => res.json()).then(data => { if(data.success) { setBalance(data.balance); setWithdrawAmount(''); alert('✅ Đã gửi yêu cầu!'); } else alert(data.message); }); };
    const handleCopyLink = () => { navigator.clipboard.writeText(`https://t.me/Dau_Tu_SWC_bot?start=${userId || 'ref'}`).then(() => alert('✅ Đã sao chép!')).catch(() => alert('❌ Lỗi!')); };
    const handleClaimMilestone = (m: number) => { fetch(`${BACKEND_URL}/api/claim-milestone`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, milestone: m }) }).then(res => res.json()).then(data => { if(data.success) { setBalance(data.balance); setMilestones((prev:any) => ({...prev, [`milestone${m}`]: true})); alert('🎉 Chúc mừng!'); } else alert(data.message); }); };
    const startTask = (t: string, u: string, d: number) => { window.open(u, '_blank'); setTaskStarted(p => ({...p, [t]: true})); setTaskTimers(p => ({...p, [t]: d})); const i = setInterval(() => setTaskTimers(p => { if (p[t as keyof typeof p] <= 1) { clearInterval(i); return {...p, [t]: 0}; } return {...p, [t]: p[t as keyof typeof p] - 1}; }), 1000); };
    const claimTaskApp = (t: string) => { if (taskTimers[t as keyof typeof taskTimers] > 0) return alert('Chờ chút!'); fetch(`${BACKEND_URL}/api/claim-app-task`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, taskType: t }) }).then(res => res.json()).then(data => { if(data.success) { setBalance(data.balance); setTasks(p => ({...p, [`${t}TaskDone`]: true})); alert(`🎉 +${data.reward} SWGT`); } else alert(data.message); }); };

    // --- HÀM XỬ LÝ ĐỔI QUÀ (VIP) MỚI ---
    const redeemItem = (itemName: string) => {
        if (window.confirm(`Xác nhận gửi yêu cầu đổi: ${itemName}?`)) {
            fetch(`${BACKEND_URL}/api/redeem`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId, itemName, cost: 0 }) // Không trừ tiền hiển thị
    ...
    const renderHeader = () => (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 20px', backgroundColor: theme.bg }}>
            {/* ... (Giữ nguyên Logo) ... */}
            <div style={{ display: 'flex', alignItems: 'center' }}> <img src="/logo.png" alt="SWC Logo" style={{ width: '45px', height: '45px', borderRadius: '50%', border: `2px solid ${theme.gold}`, marginRight: '12px', objectFit: 'cover' }} /> <div> <h1 style={{ margin: 0, fontSize: '18px', fontWeight: '900', color: theme.textLight }}>CỘNG ĐỒNG</h1> <p style={{ margin: 0, fontSize: '13px', color: theme.gold, fontWeight: 'bold' }}>Đầu tư uST</p> </div> </div>
            <div style={{ display: 'flex', alignItems: 'center', textAlign: 'right' }}>
                <div style={{ marginRight: '15px' }}> <h2 style={{ margin: 0, fontSize: '15px', color: theme.textLight, fontWeight: 'bold' }}>{userProfile.name}</h2> <p style={{ margin: 0, fontSize: '12px', color: theme.textDim, fontWeight: 'bold' }}>{getMilitaryRank(referrals)}</p> </div>
                {/* --- AVATAR CHỈNH SỬA (Căn chỉnh lại viewBox) --- */}
                <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '5px' }}>
                    <svg viewBox="-5 -5 110 110" style={{ position: 'absolute', width: '140%', height: '140%', top: '-20%', left: '-20%', zIndex: 1, pointerEvents: 'none' }}>
                        ...
                        <path d="M 20 20 Q 30 15 25 30 Q 15 25 20 20" fill={wreathColor} /> {/* Lá Trái */}
                        <path d="M 12 40 Q 25 35 20 50 Q 5 45 12 40" fill={wreathColor} />
                        <path d="M 15 65 Q 30 55 25 70 Q 10 70 15 65" fill={wreathColor} />
                        <path d="M 80 20 Q 70 15 75 30 Q 85 25 80 20" fill={wreathColor} /> {/* Lá Phải */}
                    ...
                    <div style={{ width: '52px', height: '52px', borderRadius: '50%', padding: '2px', backgroundColor: theme.bg, boxShadow: glow, zIndex: 2 }}>
                        {userProfile.photoUrl ? ( <img src={userProfile.photoUrl} alt="avatar" referrerPolicy="no-referrer" style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover', border: `2px solid ${wreathColor}` }} /> ) : ( <div style={{ width: '100%', height: '100%', borderRadius: '50%', backgroundColor: theme.cardBg, border: `2px solid ${wreathColor}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: theme.gold, fontSize: '20px' }}>👤</div> )}
                    </div>
                    <div style={{ position: 'absolute', bottom: '-10px', zIndex: 3, display: 'flex', alignItems: 'center', backgroundColor: '#000', padding: '2px 8px', borderRadius: '12px', border: `1px solid ${wreathColor}`, boxShadow: '0 2px 4px rgba(0,0,0,0.5)' }}> <span style={{ color: wreathColor, fontSize: '10px', fontWeight: 'bold', whiteSpace: 'nowrap' }}>{vipLevel}</span> </div>
    ...
    const renderRewards = () => { /* ... */ 
        // ... (Tính toán board như cũ) ...
        return (
            <div style={{ padding: '0 20px 20px 20px', paddingBottom: '100px' }}>
                {/* ... (Header, Input Code, Copy Link như cũ) ... */}
                <div style={{ textAlign: 'center', marginBottom: '25px' }}> <div style={{ fontSize: '45px', marginBottom: '5px' }}>🎁</div> <h2 style={{ color: theme.gold, margin: '0 0 5px 0', fontSize: '22px', fontWeight: '900' }}>Trung Tâm Thu Nhập</h2> <p style={{ color: theme.textDim, fontSize: '14px', margin: 0 }}>Xây dựng hệ thống - Tạo thu nhập thụ động</p> </div>
                <div style={{ backgroundColor: theme.cardBg, borderRadius: '15px', padding: '20px', marginBottom: '25px', border: `1px solid ${theme.border}` }}> <h3 style={{ margin: '0 0 15px 0', color: theme.textLight, fontSize: '16px' }}>🎟️ Nhập Mã Quà Tặng (Giftcode)</h3> <div style={{ display: 'flex', gap: '10px' }}> <input value={giftCodeInput} onChange={(e) => setGiftCodeInput(e.target.value)} placeholder="Nhập mã săn được từ Group..." style={{ flex: 1, padding: '14px', borderRadius: '10px', border: `1px solid ${theme.green}`, backgroundColor: '#000', color: theme.gold, boxSizing: 'border-box', fontSize: '14px', textTransform: 'uppercase' }} /> <button onClick={handleClaimGiftCode} style={{ backgroundColor: theme.green, color: '#fff', padding: '0 20px', borderRadius: '10px', fontWeight: 'bold', border: 'none', fontSize: '14px', cursor: 'pointer' }}>NHẬN</button> </div> </div>
                <div style={{ backgroundColor: theme.cardBg, borderRadius: '15px', padding: '20px', marginBottom: '25px', border: `1px solid ${theme.border}` }}> <h3 style={{ margin: '0 0 15px 0', color: theme.textLight, fontSize: '16px' }}>🔗 Công cụ lan tỏa</h3> <div style={{ backgroundColor: '#000', padding: '15px', borderRadius: '8px', color: theme.gold, fontSize: '15px', wordBreak: 'break-all', marginBottom: '15px', border: `1px dashed ${theme.border}` }}> https://t.me/Dau_Tu_SWC_bot?start={userId || 'ref'} </div> <div style={{ display: 'flex', gap: '10px' }}> <button onClick={handleCopyLink} style={{ flex: 1, backgroundColor: theme.gold, color: '#000', padding: '14px 0', borderRadius: '10px', fontWeight: 'bold', border: 'none', fontSize: '14px', cursor: 'pointer', textAlign: 'center' }}>📋 COPY LINK</button> <a href={`https://t.me/share/url?url=https://t.me/Dau_Tu_SWC_bot?start=${userId}&text=Vào%20nhận%20ngay%20SWGT%20miễn%20phí%20từ%20hệ%20sinh%20thái%20công%20nghệ%20uST%20này%20anh%20em!`} target="_blank" rel="noreferrer" style={{ flex: 1, backgroundColor: '#5E92F3', color: '#fff', padding: '14px 0', borderRadius: '10px', fontWeight: 'bold', border: 'none', fontSize: '14px', textAlign: 'center', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✈️ GỬI BẠN BÈ</a> </div> </div>
                ...
                <div style={{ backgroundColor: theme.cardBg, padding: '20px', borderRadius: '15px', marginBottom: '15px', border: `1px solid ${theme.border}`}}>
                    <h4 style={{margin: '0 0 8px 0', color: theme.gold, fontSize: '16px'}}>🎟️ Phiếu Đầu Tư Ưu Đãi</h4>
