...
function App() {
    ...
    const handleSpin = () => {
        if (balance < 20) return alert("⚠️ Bạn không đủ 20 SWGT để mua vé quay!");
        if (isSpinning) return;

        setIsSpinning(true);
        setSpinResultMsg('Đang quay...');
            ...
            setTimeout(() => {
                ...
                if (data.success) {
                    ...
                    if (data.rewardType === 'item') {
                        ...
                        alert(`💰 Chúc mừng! Bạn quay trúng ${data.rewardName}! Số dư đã được cập nhật.`);
                    } else {
                        alert(`😭 Rất tiếc, bạn quay trúng ô mất lượt. Chúc bạn may mắn lần sau!`);
                    }
                } else {
                    setSpinResultMsg('Lỗi quay thưởng!');
                    alert(data.message);
                }
            }, 1500);
        })
    ...
    const renderHome = () => (
        <div style={{ padding: '0 20px 20px 20px' }}>
            ...
            <div style={{ backgroundColor: theme.cardBg, borderRadius: '15px', padding: '20px', textAlign: 'center', border: `1px solid ${theme.border}`, marginBottom: '20px' }}>
                ...
                {/* --- KHÓA NÚT ĐIỂM DANH --- */}
                <button 
                    onClick={() => alert("⛔ Chức năng Điểm danh đang tạm thời bảo trì!")}
                    style={{ width: '100%', backgroundColor: '#333', color: theme.textDim, padding: '14px', borderRadius: '10px', fontWeight: 'bold', border: 'none', cursor: 'not-allowed', fontSize: '15px', transition: 'all 0.3s' }}
                >
                    ⛔ TẠM KHÓA ĐIỂM DANH
                </button>
            </div>
    ...
    const renderRewards = () => {
        ...
        return (
            <div style={{ padding: '0 20px 20px 20px', paddingBottom: '100px' }}>
                ...
                {/* 0. MINI GAME: VÒNG QUAY NHÂN PHẨM (VẪN MỞ ĐỂ KHÁCH CHƠI) */}
                <div style={{ backgroundColor: '#1C1C1E', borderRadius: '15px', padding: '20px', border: `2px solid ${theme.gold}`, marginBottom: '30px', textAlign: 'center', boxShadow: '0 0 20px rgba(244, 208, 63, 0.2)' }}>
                    ...
                    <h2 style={{ color: theme.gold, margin: '0 0 5px 0', fontSize: '18px', fontWeight: '900', textTransform: 'uppercase' }}>Vòng Quay Nhân Phẩm</h2>
                    <p style={{ color: theme.textLight, fontSize: '13px', margin: '0 0 15px 0', lineHeight: '1.5' }}>
                        Thử vận may - Rinh ngay Jackpot!<br/>Phí quay: <b style={{color: theme.gold}}>20 SWGT/lượt</b>.
                    </p>
                    ...
                    <div style={{ backgroundColor: '#000', padding: '10px', borderRadius: '8px', marginBottom: '15px', border: `1px dashed ${theme.border}` }}>
                        <p style={{ color: theme.green, fontSize: '12px', margin: 0, fontWeight: 'bold' }}>
                            {spinResultMsg || "Giải thưởng: Lên đến 100 SWGT hoặc Combo Sách VIP!"}
                        </p>
                    </div>
                ...
                {/* 1. KHU VỰC KHO TRI THỨC (CỬA HÀNG - VẪN MỞ) */}
                ...
                {/* --- KHÓA MÃ GIFTCODE VÀ LINK MỜI --- */}
                <div style={{ backgroundColor: theme.cardBg, borderRadius: '15px', padding: '20px', marginBottom: '25px', border: `1px solid ${theme.border}` }}>
                    <h3 style={{ margin: '0 0 15px 0', color: theme.textLight, fontSize: '16px' }}>🔗 Link Lan Tỏa (Đang bảo trì)</h3>
                    <div style={{ backgroundColor: '#000', padding: '15px', borderRadius: '8px', color: theme.textDim, fontSize: '13px', textAlign: 'center', marginBottom: '15px', border: `1px dashed ${theme.red}` }}>
                        Tính năng mời bạn bè đang được BQT tạm khóa để kiểm tra và đối soát dữ liệu sau đợt thanh trừng.
                    </div>
                    <button disabled style={{ width: '100%', backgroundColor: '#333', color: theme.textDim, padding: '14px 0', borderRadius: '10px', fontWeight: 'bold', border: 'none', cursor: 'not-allowed' }}>⛔ TẠM KHÓA LINK MỜI</button>
                </div>
                ...
                <div style={{ textAlign: 'center', paddingTop: '5px', marginBottom: '25px' }}>
                    <button disabled style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', backgroundColor: '#333', color: theme.textDim, padding: '14px 0', borderRadius: '10px', fontWeight: 'bold', border: 'none', fontSize: '14px', cursor: 'not-allowed' }}>
                        ⛔ TẠM KHÓA TÍNH NĂNG CHIA SẺ
                    </button>
                </div>
            </div>
        );
                ...
                <div style={{ backgroundColor: theme.cardBg, borderRadius: '15px', padding: '20px', marginBottom: '20px', border: `1px solid ${theme.border}` }}>
                    ...
                    {isUnlocked ? (
                        <div style={{ padding: '15px', backgroundColor: 'rgba(52, 199, 89, 0.1)', border: `1px dashed ${theme.green}`, borderRadius: '10px', color: theme.green, fontWeight: 'bold', fontSize: '16px', textAlign: 'center' }}>
                            🎉 ĐÃ ĐỦ ĐIỀU KIỆN. CỔNG GIAO DỊCH SẼ SỚM MỞ!
                        </div>
                    ) : (
                        <div style={{ backgroundColor: '#000', padding: '20px', borderRadius: '10px', textAlign: 'center', border: `1px solid ${theme.border}` }}>
                            ...
                            <p style={{ color: theme.gold, fontSize: '12px', margin: '10px 0 0 0', fontStyle: 'italic' }}>Tất cả giao dịch đang tạm dừng để đối soát dữ liệu!</p>
                        </div>
                    )}
                </div>

                {/* --- 3. THÔNG BÁO BẢO TRÌ GIAO DỊCH ĐÈ LÊN FORM NẠP/RÚT --- */}
                <div style={{ animation: 'fadeIn 0.5s ease' }}>
                    <div style={{ backgroundColor: 'rgba(255, 59, 48, 0.05)', border: `1px dashed ${theme.red}`, borderRadius: '15px', padding: '25px 20px', textAlign: 'center', marginBottom: '25px' }}>
                        <div style={{ fontSize: '40px', marginBottom: '10px' }}>🚧</div>
                        <h3 style={{ margin: '0 0 15px 0', color: theme.red, fontSize: '16px', fontWeight: '900', textTransform: 'uppercase' }}>
                            CỔNG GIAO DỊCH TẠM ĐÓNG
                        </h3>
                        <p style={{ margin: 0, color: theme.textLight, fontSize: '14px', lineHeight: '1.6' }}>
                            Các chức năng <b>Rút Token, Thanh khoản VNĐ và Ghép vốn</b> đang được BQT tạm thời khóa lại để tiến hành tổng kiểm toán hệ thống sau đợt thanh trừng.<br/><br/>
                            Mọi dữ liệu tài sản của bạn vẫn an toàn tuyệt đối. Cổng giao dịch sẽ sớm được mở lại. Cảm ơn sự kiên nhẫn của bạn!
                        </p>
                    </div>
                </div>

                {/* 4. KHỐI THIẾT LẬP THANH TOÁN (VẪN CHO PHÉP CẬP NHẬT THÔNG TIN) */}
                <div style={{ backgroundColor: theme.cardBg, borderRadius: '15px', padding: '20px', marginBottom: '25px', border: `1px solid ${theme.border}` }}>
